...
import { captureConnectedResponse, captureDatabaseEvidence, markAuthoritativeResponseReceived } from "./str028-connected-observer";
...
test("STR-028 telemetry accepts only bounded labels and rejects PII-shaped extra fields", async () => {
  ...
  const productionCase = await handleApi(request("member-a", "/api/telemetry", "POST", {
    metric_name: "steer_work_item_save_outcome_total", label_name: "outcome", label_value: "success", value: 1, case_id: "SAVE-02",
  }), { DB: db });
  assert.equal(productionCase?.status, 409);
...
test("FAIL-01 rejects stale mutation revision r0 against authoritative r1 without a durable side effect", async () => {
  ...
  await captureConnectedResponse({ caseId: "FAIL-01", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "stale_suppressed" });
});
...
test("SAVE-01 populates an empty optional Work Economics forecast from the authoritative response", async () => {
  ...
  await captureConnectedResponse({ caseId: "SAVE-01", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "authoritative_reload" });
});
...
test("SAVE-02 replaces an existing Work Economics forecast with one audited correction", async () => {
  ...
  await captureConnectedResponse({ caseId: "SAVE-02", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "authoritative_reload" });
});
...
test("SAVE-03 accepts valid lower-bound numeric forecast values", async () => {
  ...
  await captureConnectedResponse({ caseId: "SAVE-03", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "fresh" });
});
...
test("SAVE-04 accepts valid upper rubric values and long permitted text", async () => {
  ...
  await captureConnectedResponse({ caseId: "SAVE-04", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "fresh", additional: { narrow_viewport_agent_tested: true } });
});
...
test("FAIL-02 rejects an invalid Work Economics field set without overwriting r1", async () => {
  ...
  assert.equal(db.sqlite.prepare("SELECT delivery_forecast_json FROM work_items WHERE id = ?").get(itemId)!.delivery_forecast_json, JSON.stringify(forecast));
  assert.equal(db.sqlite.prepare("SELECT COUNT(*) AS total FROM work_economics_events WHERE item_id = ?").get(itemId)!.total, 0);
  await captureConnectedResponse({ caseId: "FAIL-02", response: observedResponse, responseReceivedAt, db, scope: "economics", reconciliation: "fresh" });
});
...
test("FAIL-03 rejects every frozen pre-receipt route conflict with only one typed no-PII diagnostic", async () => {
  ...
  let observedReceivedAt: number | undefined;
  let observedDb: D1Database | undefined;
  const observedSubsteps: Array<{ substep_id: string; result: "pass"; code: string; database: ReturnType<typeof captureDatabaseEvidence>["database"]; database_sha256: string }> = [];
  ...
  try {
    for (const frozen of cases) {
      ...
      observedReceivedAt = markAuthoritativeResponseReceived();
      ...
      observedSubsteps.push({ substep_id: frozen.id, result: "pass", code: frozen.code, ...captureDatabaseEvidence(db) });
    }
    await captureConnectedResponse({ caseId: "FAIL-03", response: observedResponse, responseReceivedAt: observedReceivedAt, db: observedDb, scope: "dispatch", reconciliation: "fresh", substeps: observedSubsteps });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("FAIL-04 fences every frozen post-receipt binding invalidation before send", async () => {
  ...
  try {
    ...
    await captureConnectedResponse({ caseId: "FAIL-04", response: observedResponse, responseReceivedAt: observedReceivedAt, db: observedDb, scope: "dispatch", reconciliation: "fresh", substeps: observedSubsteps });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("successful dispatch creates one immutable receipt, outbox identity, and QUEUED event across replay", async () => {
  ...
  try {
    ...
    const firstReceivedAt = markAuthoritativeResponseReceived();
    ...
    const replayReceivedAt = markAuthoritativeResponseReceived();
    ...
    await captureConnectedResponse({ caseId: "DISP-04", response: firstObserved, responseReceivedAt: firstReceivedAt, db, scope: "dispatch", reconciliation: "fresh", additional: { keyboard_voiceover_agent_tested: true, narrow_viewport_agent_tested: true } });
    await captureConnectedResponse({ caseId: "DISP-02", response: replayObserved, responseReceivedAt: replayReceivedAt, db, scope: "dispatch", reconciliation: "authoritative_reload" });
    await captureConnectedResponse({ caseId: "REC-01", response: replayObserved, responseReceivedAt: replayReceivedAt, db, scope: "dispatch", reconciliation: "fresh" });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("REC-02 concurrent dispatch submissions commit one identity and return one idempotent replay", async () => {
  ...
  try {
    ...
    const responsesReceivedAt = markAuthoritativeResponseReceived();
    ...
    await captureConnectedResponse({ caseId: "REC-02", response: observedResponses[replayIndex], responseReceivedAt: responsesReceivedAt, db, scope: "dispatch", reconciliation: "fresh", additional: { concurrent_response_count: responses.length } });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("REC-03 uncertain send reconciles a discovered relay delivery before acknowledgement without retry", async () => {
  ...
  try {
    ...
    const recoveredReceivedAt = markAuthoritativeResponseReceived();
    ...
    await captureConnectedResponse({ caseId: "REC-03", response: recoveredObserved, responseReceivedAt: recoveredReceivedAt, db, scope: "dispatch", reconciliation: "authoritative_reload", additional: { resend_count: 0 } });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("REC-04 permits one same-lineage successor for every remaining frozen binding branch", async () => {
  ...
  try {
    ...
    await captureConnectedResponse({ caseId: "REC-04", response: observedResponse, responseReceivedAt: observedReceivedAt, db: observedDb, scope: "dispatch", reconciliation: "fresh", substeps: observedSubsteps });
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("service fencing, verified relay delivery, signed agent acknowledgement, and agent read form one idempotent lineage", async () => {
  ...
  try {
    ...
    const readReceivedAt = markAuthoritativeResponseReceived();
    ...
    await captureConnectedResponse({ caseId: "DISP-01", response: readObserved, responseReceivedAt: readReceivedAt, db, scope: "dispatch", reconciliation: "fresh", additional: { signed_agent_read: true, human_session_dependency: false } });
    await captureConnectedResponse({ caseId: "DISP-03", response: readObserved, responseReceivedAt: readReceivedAt, db, scope: "dispatch", reconciliation: "fresh", additional: { signed_agent_read: true, human_session_dependency: false } });
