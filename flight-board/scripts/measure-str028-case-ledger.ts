...
const definitions: CaseDefinition[] = [
  ...
  { caseId: "FAIL-01", seed: "mutation r0; server r1", evidence: "FAIL-01 rejects stale mutation revision r0 against authoritative r1 without a durable side effect", authority: "server r1 and audit counts unchanged", outcome: "conflict", reconciliation: "stale_suppressed", transport: { http_status: 409, typed_code: "STALE_REVISION" } },
...
async function executeOracle(definition: CaseDefinition) {
  ...
  const substepsPass = expectedSubsteps.join("|") === observedSubsteps.join("|") && observation.substeps.every((substep) => substep.result === "pass" && Boolean(substep.database_sha256));
  ...
  const typedCode = observation.authoritative_response.code
    ?? (observation.authoritative_response.idempotent_replay ? "IDEMPOTENT_REPLAY"
      : observation.authoritative_response.state
        ?? (observation.derived.outcome === "queued" ? "QUEUED"
  ...
  return {
    ...
    action_identity: observation.authoritative_response.response_identity,
