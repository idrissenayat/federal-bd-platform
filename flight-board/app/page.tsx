...
export function WorkEconomicsPanel({ item, events, members, serviceLevels, currentUserId, saving, onSave }: {
  ...
  function submitValue(event: FormEvent<HTMLFormElement>) {
    ...
    const record: ValueHypothesis = {
      ...
      outcomeOwnerId: String(form.get("outcomeOwnerId") ?? currentUserId), strategicAlignment: String(form.get("strategicAlignment") ?? ""), confidence: String(form.get("confidence") ?? "low") as ValueHypothesis["confidence"], evidence: String(form.get("evidence") ?? ""), evidenceStatus: "verified", assumptions: String(form.get("assumptions") ?? ""), currency: String(form.get("currency") ?? ""), period: String(form.get("period") ?? ""), advisory: value?.advisory ?? null, acceptanceState: value?.advisory ? String(form.get("acceptanceState") ?? "human accepted") as ValueHypothesis["acceptanceState"] : "no proposal", acceptedBy: value?.acceptedBy ?? "", acceptedAt: value?.acceptedAt ?? "",
    };
  ...
  function submitForecast(event: FormEvent<HTMLFormElement>) {
    ...
    const record: DeliveryForecast = {
      sizeBand: String(form.get("sizeBand") ?? "M") as DeliveryForecast["sizeBand"], humanEffortRanges: [{ role: String(form.get("humanRole") ?? "Delivery roles"), minMinutes: numeric(form, "humanMinutesMin"), maxMinutes: numeric(form, "humanMinutesMax") }, ...(forecast?.humanEffortRanges.slice(1) ?? [])],
      agentCostRanges: [{ provider: String(form.get("provider") ?? "OpenAI"), minCost: numeric(form, "agentCostMin"), maxCost: numeric(form, "agentCostMax"), currency: String(form.get("currency") ?? "USD"), expectedAttempts: numeric(form, "expectedAttempts", 1) }, ...(forecast?.agentCostRanges.slice(1) ?? [])], complexity: numeric(form, "complexity", 3), uncertainty: numeric(form, "uncertainty", 3), coordination: numeric(form, "coordination", 3),
  ...
  function submitActual(event: FormEvent<HTMLFormElement>) {
    ...
    const record: ActualEconomics = {
      ...
      telemetrySource: String(form.get("telemetrySource") ?? ""), completeness: String(form.get("completeness") ?? "missing") as ActualEconomics["completeness"], completionAt: actual?.completionAt ?? null, likelyVarianceMinutes: actual?.likelyVarianceMinutes ?? null, correctedBy: actual?.correctedBy ?? "", correctedAt: actual?.correctedAt ?? "", correctionReason: String(form.get("correctionReason") ?? ""),
    };
  ...
  return <section className="work-economics" aria-labelledby={`economics-${item.id}`}>
    ...
    <div className="economics-authority" role="status"><strong>◇ AI is advisory; a named human accepts every authoritative record.</strong>{aiAdvisory ? <><p>{aiAdvisory.recommendation}</p><small>Drivers: {aiAdvisory.drivers.join(", ") || "none recorded"} · Omissions: {aiAdvisory.omissions.join(", ") || "none recorded"} · Human state: {value?.acceptanceState ?? forecast?.acceptanceState ?? outcome?.acceptanceState}</small></> : <p>No AI proposal is available. The responsible human must supply evidence and reasoning; the platform will not infer a recommendation.</p>}</div>
    ...
    <details className="economics-record" open={!forecast || economics.forecast.state !== "on track"}><summary><span>02</span><div><strong>Delivery forecast</strong><small>{forecast ? `${forecast.sizeBand} · ${economics.forecast.likelyWindow}` : "Unknown · assigned delivery owner must accept a range"}</small></div><b>{economics.forecast.state}</b></summary><form onSubmit={submitForecast}>
      <div className="system-guidance" role="status"><strong>{applicableServiceLevel ? `Same-POD ${item.workflow} service level · n=${applicableServiceLevel.sampleSize}` : "Comparable history is not yet sufficient"}</strong><p>{applicableServiceLevel ? `Observed cycle distribution: ${applicableServiceLevel.lowHours}–${applicableServiceLevel.highHours} hours, median ${applicableServiceLevel.medianHours} hours, P${applicableServiceLevel.percentile}. The owner must still accept or edit the forecast.` : "Use expert judgment with low confidence. The platform will show a same-POD/work-type distribution only after five completed observations."}</p></div>
      ...
      <div className="economics-form-grid"><label>Size band<select name="sizeBand" defaultValue={forecast?.sizeBand ?? "M"}>{["XS", "S", "M", "L", "XL"].map((option) => <option key={option}>{option}</option>)}</select></label><label>Human role aggregate<select name="humanRole" defaultValue={forecastHuman?.role ?? "Delivery roles"}>{["Product Lead", "Tech Lead", "Delivery", "Delivery roles", "Product Designer", "Platform / Ops", "Security", "Privacy / Legal", "Observe / Learn", "Builder", "Test", "Critic", "Documentation", "Operations", "Architecture"].map((role) => <option key={role}>{role}</option>)}</select></label><label>Human minutes min<input name="humanMinutesMin" type="number" min="0" defaultValue={forecastHuman?.minMinutes ?? 0} required /></label><label>Human minutes max<input name="humanMinutesMax" type="number" min="0" defaultValue={forecastHuman?.maxMinutes ?? 0} required /></label><label>Provider<input name="provider" defaultValue={forecastAgent?.provider ?? "OpenAI"} required /></label><label>Agent cost min<input name="agentCostMin" type="number" min="0" step="0.01" defaultValue={forecastAgent?.minCost ?? 0} required /></label><label>Agent cost max<input name="agentCostMax" type="number" min="0" step="0.01" defaultValue={forecastAgent?.maxCost ?? 0} required /></label><label>Currency<input name="currency" defaultValue={forecastAgent?.currency ?? "USD"} required /></label><label>Expected attempts<input name="expectedAttempts" type="number" min="0" defaultValue={forecastAgent?.expectedAttempts ?? 1} required /></label>{(["complexity", "uncertainty", "coordination"] as const).map((name) => <label key={name}>{name[0].toUpperCase() + name.slice(1)} 1–5<input name={name} type="number" min="1" max="5" defaultValue={forecast?.[name] ?? 3} required /></label>)}<label>Basis kind<select name="basisKind" defaultValue={forecast?.basisKind ?? "expert judgment"}><option>expert judgment</option><option>comparable history</option></select></label><label>Confidence<select name="confidence" defaultValue={forecast?.confidence ?? "low"}>{["low", "medium", "high"].map((option) => <option key={option}>{option}</option>)}</select></label>{forecast?.advisory && <label>AI proposal ruling<select name="acceptanceState" defaultValue={forecast.acceptanceState === "human edited" ? "human edited" : "human accepted"}><option value="human accepted">Accept unchanged</option><option value="human edited">Accept with human edits</option></select></label>}<label className="span-two">Basis / evidence<input name="basis" defaultValue={forecast?.basis ?? "Expert judgment; insufficient same-POD comparable history"} required /></label><label className="span-two">Comparable items<input name="comparableItems" defaultValue={forecast?.comparableItems ?? "None yet"} /></label><label>Earliest completion<input name="earliestCompletion" type="datetime-local" defaultValue={datetimeLocal(forecast?.earliestCompletion)} required /></label><label>Likely completion<input name="likelyCompletion" type="datetime-local" defaultValue={datetimeLocal(forecast?.likelyCompletion)} required /></label><label>Latest completion<input name="latestCompletion" type="datetime-local" defaultValue={datetimeLocal(forecast?.latestCompletion)} required /></label><label>Freshness hours<input name="freshnessHours" type="number" min="1" max="168" defaultValue={forecast?.freshnessHours ?? 24} required /></label><label>Next milestone<input name="nextMilestone" defaultValue={forecast?.nextMilestone ?? ""} required /></label><label>Milestone target<input name="nextMilestoneAt" type="datetime-local" defaultValue={datetimeLocal(forecast?.nextMilestoneAt)} required /></label><label>Phase exit<input name="phaseExit" defaultValue={forecast?.phaseExit ?? "Moving to Evaluate / QA"} required /></label><label>Phase-exit target<input name="phaseExitAt" type="datetime-local" defaultValue={datetimeLocal(forecast?.phaseExitAt)} required /></label><label>Agent work completed<input name="agentWorkCompletedAt" type="datetime-local" defaultValue={datetimeLocal(forecast?.agentWorkCompletedAt)} /></label><label>Human decision target<input name="humanDecisionTargetAt" type="datetime-local" defaultValue={datetimeLocal(forecast?.humanDecisionTargetAt)} /></label>{item.state === "blocked" && <><label>Unblock owner<input name="unblockOwner" defaultValue={forecast?.unblockOwner ?? "Named delivery owner"} required /></label><label>Unblock action<input name="unblockAction" defaultValue={forecast?.unblockAction ?? item.next_action} required /></label><label className="span-two">Cannot forecast until dependency<input name="cannotForecastUntil" defaultValue={forecast?.cannotForecastUntil ?? `Cannot forecast until: ${item.next_action}`} required /></label></>}<label className="span-two">Forecast / reforecast reason<input name="changeReason" defaultValue={forecast?.reforecastRequiredReason ? `Reforecast after ${forecast.reforecastRequiredReason}` : forecast?.changeReason ?? "Initial owner forecast accepted before execution"} required /></label></div><button disabled={saving}>{saving ? "Saving…" : forecast ? "Accept revised forecast" : "Accept delivery forecast"}</button>
    </form></details>
...
function FlowPulse({ items, activity, generatedAt, pullForecast, onOpen }: { items: WorkItem[]; activity: Activity[]; generatedAt: string; pullForecast: PullForecast; onOpen: (item: WorkItem) => void }) {
  ...
  return <section className={`flow-pulse ${pullAllowed ? "flow-open" : "flow-full"}`} aria-label="Team flow pulse">
    ...
    <div className="flow-next"><div><span>Next expected event</span><strong>{nextEvent}</strong><small>{nextItem ? `${nextItem.key} · Owner: ${nextOwner}` : nextOwner}</small></div>{nextItem && <button onClick={() => onOpen(nextItem)}>Open {nextItem.key} →</button>}<aside className={`pull-forecast pull-${pullForecast.status}`}><span>{pullForecast.headline}</span><p>{pullForecast.detail}</p>{pullForecast.missingOwners.length > 0 && <small>Forecast updates needed from: {pullForecast.missingOwners.join(", ")}</small>}<details><summary>Contributing WIP items and ranges</summary>{pullForecast.contributors.length ? pullForecast.contributors.map((entry) => <small key={entry.itemKey}>{entry.itemKey} · {entry.owner} · {entry.earliest && entry.latest ? `${formatDate(entry.earliest)} – ${formatDate(entry.latest)}` : "unknown range"} · {entry.state}/{entry.confidence}</small>) : <small>No active contributors; capacity is available now.</small>}</details></aside></div>
  </section>;
}
