...
export function WorkEconomicsPanel({ item, events, members, serviceLevels, currentUserId, saving, onSave }: {
  ...
  function submitValue(event: FormEvent<HTMLFormElement>) {
    ...
    const record: ValueHypothesis = {
      ...
      outcomeOwnerId: String(form.get("outcomeOwnerId") ?? currentUserId), strategicAlignment: String(form.get("strategicAlignment") ?? ""), confidence: String(form.get("confidence") ?? "low") as ValueHypothesis["confidence"], evidence: String(form.get("evidence") ?? ""), evidenceStatus: "unverified", evidenceRevision: "", evidenceSha256: "", evidenceVerifiedAt: "", valueMode: String(form.get("valueMode") ?? "non-monetary") as ValueHypothesis["valueMode"], assumptions: String(form.get("assumptions") ?? ""), currency: String(form.get("currency") ?? ""), period: String(form.get("period") ?? ""), advisory: value?.advisory ?? null, acceptanceState: value?.advisory ? String(form.get("acceptanceState") ?? "human accepted") as ValueHypothesis["acceptanceState"] : "no proposal", acceptedBy: value?.acceptedBy ?? "", acceptedAt: value?.acceptedAt ?? "",
    };
  ...
  const applicableServiceLevel = serviceLevels.find((entry) => entry.podId === item.pod_id && entry.workType === item.workflow) ?? null;
  ...
  return <section className="work-economics" aria-labelledby={`economics-${item.id}`}>
    ...
    <details className="economics-record" open={!value}><summary><span>01</span><div><strong>Value hypothesis</strong><small>{value ? `${value.outcomeMetric}: ${value.baseline} → ${value.target} ${value.unit}` : "Unknown · Product Lead must accept before Gate 1"}</small></div><b>{value?.confidence ?? "unknown"}</b></summary><form onSubmit={submitValue}>
      <RecordAdvisory advisory={value?.advisory} acceptanceState={value?.acceptanceState} />
      <div className="economics-form-grid economics-governance-row"><label>Value treatment<select name="valueMode" defaultValue={value?.valueMode ?? "non-monetary"}><option value="non-monetary">Non-monetary native unit</option><option value="monetary">Monetary · currency and period required</option></select></label></div>
    ...
    <details className="economics-record" open={!forecast || economics.forecast.state !== "on track"}><summary><span>02</span><div><strong>Delivery forecast</strong><small>{forecast ? `${forecast.sizeBand} · ${economics.forecast.likelyWindow}` : "Unknown · assigned delivery owner must accept a range"}</small></div><b>{economics.forecast.state}</b></summary><form onSubmit={submitForecast}>
      <RecordAdvisory advisory={forecast?.advisory} acceptanceState={forecast?.acceptanceState} />
