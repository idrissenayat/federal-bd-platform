...
16. **SB-16 Accessibility.** The complete eight-state evidence matrix—initial loading, populated/all groups, selected group, no-match, appended page, permission failure, network/retry failure, and empty POD—passes automated axe plus agent-operated keyboard, focus, screen-reader, contrast, 320 CSS-pixel, and true 200% browser-zoom checks. Every state naturally reachable by the sole enrolled owner executes on the exact hosted target. A state that canonical owner-only staging cannot naturally produce may instead be proven by the exact staged-source isolated UI renderer together with the corresponding real API/D1 authorization, empty-data, or client-error contract, provided the evidence binds the exact staged source and discloses why the hosted state is unreachable. No hidden fault injector, identity bypass, test-only product route, or production branch may be added to manufacture a state. Status/count changes are announced once; focus remains predictable after filter, refresh, append, workspace close, and error retry; no control relies on color alone.
...
20. **SB-20 Signal-scoped rollback.** The exact staging target is deployed, an active continuation/search/workspace view is captured, the prior staging version is restored, and then the exact target is restored. While old code is live, the removed endpoint/UI fails honestly without signal, work, or authority mutation; after restoration, the same authorized records and counts return. Before/during/after canonical SHA-256 projections cover every issue #70 signal table, and the issue #78 index/schema inventory, deployment IDs/statuses, client outcomes, and production non-mutation are durable. Unrelated work/authority integrity is proven by the read-only implementation boundary, zero-write telemetry, and complete green repository regression suite; exhaustive row publication or hashing of unrelated tables is not required by this signal-scoped change.
...
The implementation evidence must contain one durable matrix binding the exact staging deployment/source/environment revision and, for each state, the execution surface, automated result, relevant keyboard/focus path, announcement outcome, contrast result, viewport/zoom result, and pass/fail outcome. For an unreachable hosted state, the matrix must name the exact source-bound isolated renderer and the real API/D1 or client contract paired with it. Screenshots may supplement but do not replace the structured record. The tester is an agent; the Product Lead confirms the agent result and is not required to manually operate the matrix.
...
- All continuation chains, the 30-case latency sample, 100-concurrency sample, capture/refresh case, and every naturally reachable UI state execute against the exact staging target. Only an SB-16 state proven unreachable for the sole enrolled owner may use the exact staged-source isolated UI renderer paired with its real API/D1 or client contract; isolated evidence cannot replace any naturally reachable hosted state.
...
- Capture per-table canonical projections and SHA-256 hashes for every issue #70 signal table before target, during prior-version rollback, and after target restoration. Boolean equality without the underlying bounded projection/digest is insufficient. Record the issue #78 schema/index inventory. For unrelated work/authority data, prove the read-only boundary through implementation inspection, zero-write telemetry, and the complete green repository regression suite rather than exporting or hashing unrelated rows.
