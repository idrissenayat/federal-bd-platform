...
| 2026-08-12 | collaboration | ~~Preserve the existing Scrum-oriented Buzz/OpenProject/XWiki pilot; provision STEER in a separate workspace~~ **Superseded by the Block Buzz correction below.** | Maintain provenance and avoid presenting relabelled Scrum configuration as evidence for STEER | Superseded 2026-08-12 |
...
| 2026-08-12 | correction | “Buzz” means Block's open-source `block/buzz` agent workspace; the custom Sites app and OpenProject persona provisioning in earlier commits are superseded, not Block Buzz evidence | The requested tool was misidentified; preserving and correcting the record is more honest than rewriting history | Permanent; revisit only if the selected collaboration platform changes |
| 2026-08-12 | collaboration | The official Block Buzz local B1 communication slice passed signed identity, membership denial, revocation, restart retention, and a real Codex ACP reply; production/shared promotion remains default-closed | Local functional evidence supports the pilot but does not prove production operations or authority write-through | At production brief or B2 proposal |
| 2026-08-12 | collaboration | Promote the communication target from the local proof relay to the isolated Railway project `steer-block-buzz`; local services are evidence-only after remote cutover | Team members and agents need a remotely reachable TLS relay rather than infrastructure tied to one person's workstation | Revisit hosting after restore and cost evidence |
| 2026-08-12 | agents | Do not copy local Codex or Claude login sessions into Railway; hosted agent workers remain off until provider service credentials are entered directly in Railway and remote ACP proofs pass | A healthy relay is not authority to move personal login state or expose model credentials | When service credentials are provisioned |
| 2026-08-12 | agents | Pre-enroll the complete seven-role reference fleet in Buzz while activating workers only by need: Scout, Architect, Builder, Test Agent, Critic, Docs Agent, and Ops Agent | Distinct signed identities make role provenance and least-privilege channel boundaries visible without pretending dormant identities are running capacity | At the first fleet-cost and role-activation review |
